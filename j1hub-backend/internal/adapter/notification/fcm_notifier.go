...
func (n *fcmNotifier) Send(ctx context.Context, userID, title, body string) error {
	...
	if user.UserID == "" { // fcm_token is not in domain.User yet, let's assume we find it via repo if we add it
		// For now, let's just log or assume fcm_token is empty
		return nil
	}

	// In a real scenario, we'd have fcm_token in the user struct
	// Let's assume we added it to domain.User in Phase 3 or we fetch it here.
	// Since I didn't add it to domain.User, I'll just log for now.
