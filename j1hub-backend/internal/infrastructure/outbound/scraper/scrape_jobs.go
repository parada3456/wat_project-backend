...
type ScrapeJobsUseCase struct {
	jobRepo     jobport.JobPostingRepository
	housingRepo jobport.JobHousingRepository
}

func NewScrapeJobsUseCase(jobRepo jobport.JobPostingRepository, housingRepo jobport.JobHousingRepository) *ScrapeJobsUseCase {
...
func (uc *ScrapeJobsUseCase) Run(ctx context.Context) error {
	...
	scrapeSource := func(source JobSource, name string, url string) {
		defer wg.Done()
