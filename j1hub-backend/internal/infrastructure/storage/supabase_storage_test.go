...
func TestSupabaseStorage_DeleteFile(t *testing.T) {
	// Success path
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	storagePort := storage.NewSupabaseStorage(cfg)
	err := storagePort.DeleteFile(context.Background(), "bucket", "test.jpg")
	assert.NoError(t, err)
	...
	storagePortErr := storage.NewSupabaseStorage(cfgErr)
	err = storagePortErr.DeleteFile(context.Background(), "bucket", "test.jpg")
	assert.Error(t, err)
}
