...
type JourneyUC interface {
	ListPhases(ctx context.Context) ([]missiondomain.JourneyPhase, error)
	GetHistory(ctx context.Context, userID string) ([]missiondomain.UserPhaseHistory, error)
...
type AdvancePhaseUC interface {
	TryAdvancePhase(ctx context.Context, userID string) (*gamificationusecase.PhaseTransitionResponse, error)
}

type LeaderboardUC interface {
	GetLeaderboard(ctx context.Context, scope, jobID string) ([]gamificationusecase.LeaderboardEntry, error)
}
