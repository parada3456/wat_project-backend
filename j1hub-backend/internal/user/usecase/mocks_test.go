...
func (m *MockUserRepository) FindUserJob(ctx context.Context, userID string) (*userdomain.UserJob, error) {
	args := m.Called(ctx, userID)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
	return args.Get(0).(*userdomain.UserJob), args.Error(1)
}
func (m *MockUserRepository) FindUserJobs(ctx context.Context, userID string) ([]userdomain.UserJob, error) {
	args := m.Called(ctx, userID)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
...
func (m *MockUserRepository) AssignJob(ctx context.Context, userID, jobID string, isMain bool, startDate, endDate *time.Time) error {
