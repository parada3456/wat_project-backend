...
func TestLeaderboardUseCase_GetLeaderboard_Success(t *testing.T) {
	...
	uc := usecase.NewLeaderboardUseCase(leaderRepo, profileRepo, ubRepo)
	...
	mockProfileHidden := &domain.Profile{
		UserID:          "usr_3",
		RadarVisibility: "Hidden",
	}

	mockUserBadges := []domain.UserBadge{
		{UserID: "usr_1", BadgeID: "badge_gold"},
	}

	leaderRepo.On("FindByScope", ctx, scope, jobID).Return(mockUsers, nil)
	profileRepo.On("FindByUserID", ctx, "usr_1").Return((*domain.Profile)(nil), nil)
	profileRepo.On("FindByUserID", ctx, "usr_2").Return((*domain.Profile)(nil), nil)
	...
	ubRepo.On("FindByUser", ctx, "usr_2").Return([]domain.UserBadge{}, nil)
	ubRepo.On("FindByUser", ctx, "usr_3").Return([]domain.UserBadge{}, nil)
