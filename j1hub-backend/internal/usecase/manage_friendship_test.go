...
func TestManageFriendshipUseCase_SendRequest_Success(t *testing.T) {
	...
	uc := usecase.NewManageFriendshipUseCase(friendRepo, userRepo, notifier, clock)

	ctx := context.Background()
	senderID := "usr_aaa"
	targetID := "usr_bbb" // aaa < bbb, so u1 = aaa, u2 = bbb

	userRepo.On("FindByID", ctx, targetID).Return(&domain.User{UserID: targetID}, nil)
	friendRepo.On("FindByCanonicalPair", ctx, senderID, targetID).Return((*domain.Friendship)(nil), nil)
	friendRepo.On("Insert", ctx, mock.AnythingOfType("*domain.Friendship")).Return(nil).Run(func(args mock.Arguments) {
		f := args.Get(1).(*domain.Friendship)
...
func TestManageFriendshipUseCase_SendRequest_Duplicate(t *testing.T) {
	...
	userRepo.On("FindByID", ctx, targetID).Return(&domain.User{UserID: targetID}, nil)
	friendRepo.On("FindByCanonicalPair", ctx, senderID, targetID).Return(&domain.Friendship{FriendshipID: "frn_123"}, nil)
...
func TestManageFriendshipUseCase_RespondToRequest_Accept(t *testing.T) {
	...
	mockFriendship := &domain.Friendship{
...
func TestManageFriendshipUseCase_ListPendingRequests_Stub(t *testing.T) {
	uc := usecase.NewManageFriendshipUseCase(nil, nil, nil, &MockClock{})
